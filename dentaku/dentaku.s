...
### 3    typedef enum {
### 12    struct Token {
### 19    typedef struct Token Token;
### 22    typedef enum {
### 38    struct Node {
### 45    typedef struct Node Node;
### 48    struct LVar {
### 54    typedef struct LVar LVar;
### 120    int consume(char *op);
### 121    Token *consume_type(TokenKind tkind);
### 122    void expect(char *op);
### 123    int expect_number();
### 124    int at_eof();
### 125    Token *new_token(TokenKind kind, Token *cur, char *str);
### 126    void tokenize();
### 127    Node *new_node(NodeKind kind, Node *lhs, Node *rhs);
### 128    Node *new_node_num(int val);
### 129    LVar *find_lvar(Token *tok);
### 131    void program();
### 132    Node *stmt();
### 133    Node *expr();
### 134    Node *assign();
...
### 139    Node *unary();
### 140    Node *primary();
### 142    void gen_lval(Node *node);
### 143    void gen(Node *node);
### 146    Token *token;
  .globl token
  .data
token:
...
### 197    return token->kind == TK_EOF;
  ...
  pop rbp
  ret
### 201    Token *new_token(TokenKind kind, Token *cur, char *str) {
  .globl new_token
...
new_token:
  ...
  push rdx
  sub rsp, 8
### 202    Token *tok = calloc(1, sizeof(Token));
### 202    Token *tok = calloc(1, sizeof(Token));
  push rax
  pop rax
  mov rax, rbp
...
### 219    char *p = user_input;
  ...
  push rdi
  pop rax
### 220    Token *head = calloc(1, sizeof(Token));
### 220    Token *head = calloc(1, sizeof(Token));
  push rax
  pop rax
  mov rax, rbp
...
.Lorend_17:
  ...
  cmp rax, 0
  je  .Lendif16
### 242    cur = new_token(TK_RESERVED, cur, p);
  mov rax, rbp
  sub rax, 24
  push rax
  push r15
...
.Lorend_21:
  ...
  cmp rax, 0
  je  .Lendif20
### 253    cur = new_token(TK_RESERVED, cur, p++);
  mov rax, rbp
  sub rax, 24
  push rax
  push r15
...
### 259    q = p;
  ...
  push rdi
  pop rax
### 260    while (is_alnum(*q) && !(q == p && '0' <= *q && *q <= '9')) {
.Lbegin30:
.Lcontinue30:
  push r15
  mov rax, rbp
...
### 263    if (q > p) {
  ...
  cmp rax, 0
  je  .Lendif34
### 264    cur = new_token(TK_IDENT, cur, p);
  mov rax, rbp
  sub rax, 24
  push rax
  push r15
...
### 270    if (isdigit(*p)) {
  ...
  cmp rax, 0
  je  .Lendif35
### 271    cur = new_token(TK_NUM, cur, p);
  mov rax, rbp
  sub rax, 24
  push rax
  push r15
...
### 288    LVar *var;
  push rax
  pop rax
### 289    for (var = locals; var; var = var->next)
  mov rax, rbp
  sub rax, 16
  push rax
...
### 292    return 0;
  ...
  pop rbp
  ret
### 295    Node *new_node(NodeKind kind, Node *lhs, Node *rhs) {
  .globl new_node
...
new_node:
  ...
  push rdx
  sub rsp, 8
### 296    Node *node = calloc(1, sizeof(Node));
### 296    Node *node = calloc(1, sizeof(Node));
  push rax
  pop rax
  mov rax, rbp
...
new_node_num:
  ...
  push rdi
  sub rsp, 8
### 304    Node *node = calloc(1, sizeof(Node));
### 304    Node *node = calloc(1, sizeof(Node));
  push rax
  pop rax
  mov rax, rbp
...
.Lendif58:
  push rax
  pop rax
### 418    Token *tok = consume_type(TK_IDENT);
### 418    Token *tok = consume_type(TK_IDENT);
  push rax
  pop rax
  mov rax, rbp
...
### 431    lvar->len = tok->len;
  ...
  push rdi
  pop rax
### 432    lvar->offset = (locals ? locals->offset : 0) + 8;
  mov rax, rbp
  sub rax, 24
  push rax
  pop rax
...
.Lendif59:
  push rax
  pop rax
### 440    return new_node_num(expect_number());
  push r15
  push r15
  mov rax, 0
  mov r15, rsp
...
### 549    program();
  ...
  push rax
  pop rax
### 552    printf(".intel_syntax noprefix\n");
  push r15
